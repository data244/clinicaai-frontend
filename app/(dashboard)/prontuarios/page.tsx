...
export default function ProntuariosPage() {
  ...
  const gerarResumoIA = async (id: string) => {
    setSalvandoResumo(true); setResumoIA('')
    try {
      const res = await iaApi.gerarResumo(id)
      setResumoIA(res.resumo_ia)
      setProntuarios(prev => prev.map(p => p.id === id ? { ...p, resumo_ia: res.resumo_ia } : p))
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : 'Erro ao gerar resumo')
    } finally { setSalvandoResumo(false) }
  }
      ...
      {showForm && (
        <div className="card mb-6 border-primary-100">
          <h2 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <Mic className="w-4 h-4 text-primary-500" />Novo registro clínico
          </h2>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="label">Exame físico</label><textarea className="input resize-y" {...register('exame_fisico')} rows={2} /></div>
              <div><label className="label">Hipótese diagnóstica</label><textarea className="input resize-y" {...register('hipotese_diagnostica')} rows={2} /></div>
            </div>
            <div><label className="label">Conduta / Plano terapêutico</label><textarea className="input resize-y" {...register('conduta')} rows={3} /></div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="label">Prescrição</label><textarea className="input resize-y" {...register('prescricao')} rows={2} /></div>
              <div><label className="label">Observações</label><textarea className="input resize-y" {...register('observacoes')} rows={2} /></div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              {prontuarioAtual && (
                <button type="button" onClick={() => gerarResumoIA(prontuarioAtual)} disabled={salvandoResumo}
      ...
      <div className="space-y-2">
        ...
        {prontuarios.length === 0 && selectedPaciente && (
          <div className="card text-center py-10 text-gray-400">
            ...
            <p>Nenhum prontuário para este paciente.</p>
          </div>
        )}
        {prontuarios.map(pr => (
          <div key={pr.id} className="card hover:shadow-md transition-shadow">
            ...
            {expandedId === pr.id && (
              <div className="mt-4 pt-4 border-t border-gray-100 space-y-3 text-sm">
                ...
                {pr.exame_fisico && <div><p className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1">Exame Físico</p><p className="text-gray-700">{pr.exame_fisico}</p></div>}
                {pr.hipotese_diagnostica && <div><p className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1">Hipótese</p><p className="text-gray-700">{pr.hipotese_diagnostica}</p></div>}
                ...
                {pr.resumo_ia ? (
                  <div className="bg-purple-50 border border-purple-100 rounded-lg p-3">
                    <p className="text-xs font-semibold text-purple-600 mb-1 flex items-center gap-1"><Sparkles className="w-3 h-3" /> Resumo por IA</p>
                    <p className="text-purple-900 text-sm whitespace-pre-wrap">{pr.resumo_ia}</p>
                  </div>
                ) : (
                  <button type="button" onClick={() => gerarResumoIA(pr.id)} disabled={salvandoResumo}

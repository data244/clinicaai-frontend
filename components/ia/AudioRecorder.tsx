...
export default function AudioRecorder({ onTranscricao, disabled }: AudioRecorderProps) {
  ...
  const iniciarGravacao = useCallback(async () => {
    ...
    try {
      ...
      setErro('Permissão de microfone negada ou não suportado.')
    }
  }, [])
  ...
  return (
    <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center space-y-3">
      {estado === 'idle' && (
        <>
          <p className="text-sm text-gray-500 font-medium">Gravar ou fazer upload de áudio da consulta</p>
          ...
          <p className="text-xs text-gray-400">MP3, MP4, WAV, OGG, WebM — máx 25 MB</p>
          <input
            ref={fileInputRef}
            type="file"
      ...
      {estado === 'gravando' && (
        <>
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-gray-500 font-mono">{formatTempo(duracao)}</span>
          </div>
          <button
            type="button"
            onClick={pararGravacao}
            className="flex items-center gap-2 bg-gray-800 hover:bg-gray-900 text-white font-medium py-2 px-5 rounded-lg transition-colors mx-auto"
          >
